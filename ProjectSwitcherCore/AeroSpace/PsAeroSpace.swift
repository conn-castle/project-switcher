...
struct PsWorkspaceSummary: Equatable, Sendable {
    /// Workspace name.
    let workspace: String
...
/// AeroSpace CLI wrapper for pswitcher.
public struct PsAeroSpace {
    private static let logger = Logger(subsystem: "com.projectswitcher", category: "PsAeroSpace")
    ...
    private func waitForReadiness() -> Result<Void, PsCoreError> {
    ...
    /// - Returns: Success or an error.
    public func reloadConfig() -> Result<Void, PsCoreError> {
    ...
    /// - Returns: Success or an error.
    func moveWindowToWorkspace(workspace: String, windowId: Int, focusFollows: Bool = false) -> Result<Void, PsCoreError> {
    ...
    /// - Returns: Window list or an error.
    func listWindowsFocusedMonitor() -> Result<[PsWindow], PsCoreError> {
    ...
    /// - Returns: Window list or an error.
    func listWindowsOnFocusedMonitor(appBundleId: String) -> Result<[PsWindow], PsCoreError> {
        switch runAerospace(arguments: [
            "list-windows",
            "--monitor",
            "focused",
    ...
    /// - Returns: Window list or an error.
    func listVSCodeWindowsOnFocusedMonitor() -> Result<[PsWindow], PsCoreError> {
        listWindowsOnFocusedMonitor(appBundleId: PsVSCodeLauncher.bundleId)
    }

    /// Returns Chrome windows on the focused monitor.
    /// - Returns: Window list or an error.
    func listChromeWindowsOnFocusedMonitor() -> Result<[PsWindow], PsCoreError> {
        listWindowsOnFocusedMonitor(appBundleId: PsChromeLauncher.bundleId)
    }
    ...
    private func runAerospace(
        arguments: [String],
        timeoutSeconds: TimeInterval = PsAeroSpace.defaultCommandTimeoutSeconds
    ...
    /// - Returns: Window list or an error.
    private func listWindowsFocused() -> Result<[PsWindow], PsCoreError> {
        switch runAerospace(arguments: [
            "list-windows",
            "--focused",
            "--format",
    ...
    /// - Returns: Success or an error.
    private func closeWindow(windowId: Int) -> Result<Void, PsCoreError> {
        guard windowId > 0 else {
            return .failure(validationError("Window ID must be positive."))
        }
    ...
    private func parseWindowSummaries(output: String) -> Result<[PsWindow], PsCoreError> {
        AeroSpaceParser.parseWindowSummaries(output: output)
    }

    private func parseWorkspaceSummaries(output: String) -> Result<[PsWorkspaceSummary], PsCoreError> {
    ...
    ///
    /// Static to avoid capturing non-Sendable `PsAeroSpace` (a struct) in the
    ...
    private static func recoveryProbeNonTimeoutFailureDetail(_ error: PsCoreError, commandRunner: CommandRunning) -> String {
        guard commandRunner is PsSystemCommandRunner else {
    ...
    private static func conciseRecoveryProbeErrorContext(_ error: PsCoreError) -> String {
    ...
    private static func recoveryProbeTimeoutSeconds(commandRunner: CommandRunning) -> TimeInterval {
        if commandRunner is PsSystemCommandRunner {
    ...
    private func commandHelpOutput(command: String) -> Result<String, PsCoreError> {
...
extension PsAeroSpace: AeroSpaceHealthChecking {
