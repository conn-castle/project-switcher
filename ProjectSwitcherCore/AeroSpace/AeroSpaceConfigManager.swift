...
//  the ProjectSwitcher-managed safe configuration.
//
...
public struct AeroSpaceConfigManager {
    /// Marker comment that identifies configs managed by ProjectSwitcher.
    static let managedByMarker = "# Managed by ProjectSwitcher - do not edit manually"
    ...
    private static var backupPath: String {
        URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
            .appendingPathComponent(".aerospace.toml.project-switcher-backup", isDirectory: false)
            .path
    }
    ...
    /// Returns true if the existing config is managed by ProjectSwitcher.
    /// - Returns: True if the config starts with the managed-by marker, false otherwise.
    private func configIsManagedByProjectSwitcher() -> Result<Bool, PsCoreError> {
    ...
    private func backupExistingConfig() -> Result<Void, PsCoreError> {
        guard configExists() else {
            return .success(())
        }
    ...
    public func writeSafeConfig() -> Result<Void, PsCoreError> {
        ...
        switch configIsManagedByProjectSwitcher() {
    ...
    public func configStatus() -> AeroSpaceConfigStatus {
        ...
        case .failure:
            return .unknown
        case .success(true):
            return .managedByProjectSwitcher
        case .success(false):
    ...
    static let versionPrefix = "# ps-config-version: "
    ...
    /// Extracts the `ps-config-version` number from config content.
    ...
    /// - `.managedByProjectSwitcher` with stale version → calls `updateManagedConfig()`, returns `.updated`.
    /// - `.managedByProjectSwitcher` with current version → returns `.alreadyCurrent`.
        ...
        case .managedByProjectSwitcher:
            ...
            guard let latestVersion else {
                return .failure(fileSystemError(
                    ...
                    detail: "The bundled aerospace-safe.toml is missing or has no ps-config-version line."
                ))
            }
