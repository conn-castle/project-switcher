...
public struct DataPaths: Sendable {
    ...
    public var primaryLogFile: URL {
        logsDirectory.appendingPathComponent(Self.currentLogFileName, isDirectory: false)
    }

    /// Returns `~/.local/state/project-switcher/logs/project-switcher.log.lock`.
    var logLockFile: URL {
        logsDirectory.appendingPathComponent(Self.currentLogLockFileName, isDirectory: false)
    }
