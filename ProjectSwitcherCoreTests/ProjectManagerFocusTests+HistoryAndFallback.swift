...
extension ProjectManagerFocusTests {
    ...
    func testActivationFailureDoesNotRollbackPush() async {
        ...
        let chromeWindow = PsWindow(
            ...
            workspace: "ps-test", windowTitle: "PS:test - Chrome"
        )
        ...
        failingIde.result = .failure(PsCoreError(category: .command, message: "IDE launch failed"))
    ...
    func testCloseProjectFallsBackToNonProjectWorkspaceWhenStackEmpty() async {
        ...
        aero.workspacesWithFocusResult = .success([
            PsWorkspaceSummary(workspace: "ps-test", isFocused: true),
            PsWorkspaceSummary(workspace: "1", isFocused: false),
            PsWorkspaceSummary(workspace: "personal", isFocused: false)
        ])
    ...
    func testCloseProjectFallsBackToCanonicalWorkspaceWhenOnlyProjectWorkspaces() async {
        ...
        aero.workspacesWithFocusResult = .success([
            PsWorkspaceSummary(workspace: "ps-test", isFocused: true),
            PsWorkspaceSummary(workspace: "ps-other", isFocused: false)
        ])
