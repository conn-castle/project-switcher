...
private enum MenuBarHealthIndicator {
    ...
    static let accessibilityDescription = "ProjectSwitcher health indicator"
...
private struct MenuItems {
    ...
    let recoverProjectSwitcher: NSMenuItem
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let immediateWindowCycleQueue = DispatchQueue(
        label: "com.projectswitcher.window-cycle-immediate",
        qos: .userInteractive
    )
    ...
    ///
    /// ProjectSwitcher is a menu bar app (`LSUIElement`) without a standard window, so reopen events
    ...
    private func completeAppSetup() {
        ...
        case .success(let result):
            if case .updated(let from, let to) = result {
                ...
                // reload calls PsSystemCommandRunner.run() which may trigger the
                // one-time login shell PATH resolution on first use.
                DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                    let aerospace = PsAeroSpace()
    ...
    private func requestAccessibilityOnFirstLaunchIfNeeded() {
        ...
        let currentBuild = (Bundle.main.infoDictionary?["CFBundleVersion"] as? String) ?? ProjectSwitcher.version
    ...
    private func ensureAeroSpaceRunning() {
        ...
        if checker.isApplicationRunning(bundleIdentifier: PsAeroSpace.bundleIdentifier) {
    ...
    private func cleanupStaleFocusScripts() {
        let binDir = URL(fileURLWithPath: NSHomeDirectory())
            .appendingPathComponent(".config/project-switcher/bin")
        for name in ["ps-focus-next", "ps-focus-prev"] {
    ...
    private func makeMenu() -> NSMenu {
        ...
        let recoverProjectSwitcherItem = NSMenuItem(
            title: "Recover Project",
            action: #selector(recoverProjectSwitcher),
            keyEquivalent: ""
        )
        recoverProjectSwitcherItem.target = self
        recoverProjectSwitcherItem.isEnabled = false // Toggled in menuNeedsUpdate
        menu.addItem(recoverProjectSwitcherItem)
        ...
        menuItems = MenuItems(
            ...
            recoverProjectSwitcher: recoverProjectSwitcherItem,
    ...
    private func makeSwitcherController() -> SwitcherPanelController {
        ...
        controller.onRecoverProjectRequested = { [weak self] focus, completion in
            guard let self else {
                completion(.failure(PsCoreError(category: .command, message: "Recover Project unavailable.")))
                return
            }

            guard let screenFrame = NSScreen.main?.visibleFrame else {
                self.logAppEvent(event: "recovery.no_screen", level: .error, message: "No primary screen available")
                let error = PsCoreError(category: .system, message: "No primary screen available")
            ...
            guard let recoveryCoord = self.recoveryOperationCoordinator else {
                let error = PsCoreError(category: .system, message: "Recovery coordinator unavailable")
                self.logAppEvent(event: "switcher.recover_project.failed", level: .error, message: error.message)
                completion(.failure(error))
                return
            }
    ...
    @objc private func showAbout() {
        ...
        NSApp.orderFrontStandardAboutPanel(options: [
            ...
            .applicationVersion: ProjectSwitcher.version
        ])
    }
...
extension AppDelegate: NSMenuDelegate {
    ...
    func menuNeedsUpdate(_ menu: NSMenu) {
        ...
        menuItems.recoverProjectSwitcher.isEnabled = hasFocus
